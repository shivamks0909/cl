-- Clean backend reset script - drops all data and recreates schema correctly

-- Drop all custom tables (in order to respect foreign keys)
...
CREATE INDEX idx_responses_clickid ON responses(clickid);
CREATE INDEX idx_responses_status ON responses(status);
CREATE INDEX idx_responses_created_at ON responses(created_at);
CREATE INDEX idx_projects_source ON projects(source);
CREATE INDEX idx_supplier_project_links_active ON supplier_project_links(supplier_id, project_id, status);
